...
def runge_kutta_6(dydt, y0, t):
    ...
    for i in range(len(t) - 1):
        dt = t[i + 1] - t[i]
        k1 = dydt(y[i], t[i])
        k2 = dydt(y[i] + (1/3) * dt * k1, t[i] + (1/3) * dt)
        k3 = dydt(y[i] + (1/6) * dt * k1 + (1/6) * dt * k2, t[i] + (1/3) * dt)
        k4 = dydt(y[i] + (1/8) * dt * k1 + (3/8) * dt * k3, t[i] + (1/2) * dt)
        k5 = dydt(y[i] + (1/2) * dt * k1 - (3/2) * dt * k3 + (2) * dt * k4, t[i] + (2/3) * dt)
        k6 = dydt(y[i] - (3/2) * dt * k1 + (3/2) * dt * k3 + dt * k5, t[i] + dt)
        y[i + 1] = y[i] + (1 / 6) * dt * (k1 + 4 * k4 + k6)
...
def calculeaza_valori_6(dydt, y0, t):
    t_span = np.linspace(0, t, 101)
    ...
    if metoda == "euler":
        ...
        y = runge_kutta(dydt, y0, t_span)
    elif metoda == "runge_kutta_6":
        y = runge_kutta_6(dydt, y0, t_span)
    else:
        print("Error:", "Metoda nevalida.")
...
def calculeaza_er_abs(y_odeint, y):
    return np.abs(y_odeint - y)

def calculeaza_er_rel(y_odeint, y):
    return np.abs(y_odeint - y) / np.abs(y_odeint)
...
def update_plot(i):
    ...
    ax.clear()
    ax.plot(data['t'], data['y_odeint'], label='odeint')
    ax.plot(data['t'], data['y-metoda'], label=metoda)
    ax.legend()
    ax.set_title('Soluția numerică')
    ax.set_xlabel('Timp')
    ax.set_ylabel('y')
...
# Adăugați variabila pentru metoda RK6
rk6_radio = tk.Radiobutton(root, text="Runge-Kutta 6", variable=metoda_var, value="runge_kutta_6")
rk6_radio.pack()
